    ...
    <div className="grid gap-5">
      ...
      <div className="overflow-x-auto rounded-lg border">
        <table className="w-full min-w-[980px] text-left text-sm">
          <thead className="bg-muted text-muted-foreground"><tr><th className="p-4 font-semibold">Contact</th><th className="p-4 font-semibold">Request</th><th className="p-4 font-semibold">Source</th><th className="p-4 font-semibold">Visitor IP</th><th className="p-4 font-semibold">Email</th><th className="p-4 font-semibold">SMS</th><th className="p-4 font-semibold">Received</th></tr></thead>
          <tbody>{visible.map((lead) => <tr key={lead.id} className="border-t align-top"><td className="p-4"><p className="font-semibold">{lead.name}</p><div className="mt-2 grid gap-1 text-xs">{lead.phone !== "0000000000" ? <a href={`tel:${lead.phone}`} className="inline-flex items-center gap-1.5 text-primary"><Phone className="size-3" aria-hidden />{lead.phone}</a> : <span className="text-muted-foreground">No phone submitted</span>}{lead.email ? <a href={`mailto:${lead.email}`} className="inline-flex items-center gap-1.5 text-primary"><Mail className="size-3" aria-hidden />{lead.email}</a> : null}</div></td><td className="p-4"><Badge>{lead.leadType || "General Contact"}</Badge><p className="mt-2 max-w-xs text-xs leading-5 text-muted-foreground">{lead.message || lead.city || "No additional message"}</p></td><td className="p-4"><p>{lead.source}</p><p className="mt-1 max-w-48 truncate text-xs text-muted-foreground">{lead.sourcePage || "Website"}</p></td><td className="p-4 font-mono text-xs text-muted-foreground">{String(lead.metadata?.visitorIp || "—")}</td><td className="p-4"><div className="grid gap-1"><Badge variant={deliveryVariant(lead.delivery.adminEmail)}>Admin: {lead.delivery.adminEmail}</Badge>{lead.email ? <Badge variant={deliveryVariant(lead.delivery.customerEmail)}>Customer: {lead.delivery.customerEmail}</Badge> : null}</div></td><td className="p-4"><div className="grid gap-1"><Badge variant={deliveryVariant(lead.delivery.adminSms)}>Admin: {lead.delivery.adminSms}</Badge><Badge variant={deliveryVariant(lead.delivery.customerSms)}>Customer: {lead.delivery.customerSms}</Badge></div></td><td className="p-4"><p>{new Intl.DateTimeFormat("en-IN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(lead.createdAt))}</p>{lead.delivery.errors.length ? <p className="mt-2 max-w-56 text-xs leading-5 text-red-600">{lead.delivery.errors.join(" · ")}</p> : null}</td></tr>)}</tbody>
        </table>
